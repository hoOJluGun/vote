...
- **Advanced CLI and Web IDE** with AI-powered capabilities
...
# System health check using dedicated script
...
The Cognitive Execution Layer comes with a powerful command-line interface that enables AI-assisted development from the terminal:

```bash
# Install the CLI globally (after building)
npm run build
npm install -g

# Or run directly from the project directory
node cli/enhanced-cli.js --help

# Interactive chat session
cel chat

# Generate code
cel generate "Create a React component for a todo list" -l javascript -o TodoList.jsx

# Refactor existing code
cel refactor src/components/Button.jsx --backup

# Explain code functionality
cel explain src/utils/helpers.js

# Run goal-oriented tasks
cel run-goal "Add authentication to the login flow"

# Perform self-healing on problematic files
cel heal src/buggy-component.js --test

# Generate smart git commits
cel commit -t feat -s auth
...
The Web IDE provides a rich, VS Code-inspired interface with integrated AI assistance:

```bash
# Using the dedicated startup script
chmod +x run-web-ide.sh
./run-web-ide.sh

# Or navigate to the web IDE directory manually
cd ide/web-ide
...
Then open `http://localhost:5173` in your browser to access the IDE.

Key features of the Web IDE include:
